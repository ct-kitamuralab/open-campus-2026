...
- [研究分野マップ](clustering/): 教員紹介文をもとに、学科・教員・研究室の研究内容の近さを可視化するページ
...
| 生成済み分析データ | `data/clustering-analysis.json` |
| 生成スクリプト | `tools/build-clustering-data.js` |
| 日本語形態素解析器 | `vendor/kuromoji.js` |

Node.js を用意したうえで、次のコマンドを実行します。

```sh
node tools/build-clustering-data.js
```
...
`main` ブランチへのプッシュ時、GitHub Actions が研究分野マップの分析データを再生成します。`data/clustering-analysis.json` に差分がある場合は、`github-actions[bot]` が更新コミットを作成します。

ワークフロー定義は `.github/workflows/build-clustering-analysis.yml` にあります。
